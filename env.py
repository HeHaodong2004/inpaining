...
class Env:
    ...
    def calculate_reward(self, dist):
        reward = 0
        reward -= dist / UPDATING_MAP_SIZE * 7
        ...
        reward += 1.2*delta_num / (SENSOR_RANGE * 3.14 // FRONTIER_CELL_SIZE)

        if not hasattr(self, 'prev_uncertainty'):
            self.prev_uncertainty = self.last_uncertainty

        diff_uncertainty = abs(self.last_uncertainty - self.prev_uncertainty)

        #GAMMA_DIFF = 1e-20 #to be tested
        GAMMA_DIFF = 0 

        reward += GAMMA_DIFF * diff_uncertainty
    ...
    def plot_env(self, step):
            ...
            plt.suptitle('Reward: {:.4g}  Explored ratio: {:.4g}  Travel distance: {:.4g}'.format(
                self.last_reward, self.explored_rate, self.travel_dist))
            plt.tight_layout()
