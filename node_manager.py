...
class NodeManager:
    ...
    def update_graph(self, robot_location, frontiers, updating_map_info):
        ...
        if self.frontier is None:
            ...
            observed_frontier = self.frontier - frontiers
            observed_our_range = []
            for frontier in observed_frontier:
                if np.linalg.norm(robot_location - np.array(frontier).reshape(2)) > SENSOR_RANGE + FRONTIER_CELL_SIZE:
                    observed_our_range.append(frontier)
            for frontier in observed_our_range:
                observed_frontier.remove(frontier)
        ...
        for coords in node_coords:
            ...
            if node is None:
                ...
                if node.utility == 0 or np.linalg.norm(node.coords - robot_location) > 2 * SENSOR_RANGE:
                    ...
                    to_update = False
                    if len(observed_frontier) > 0:
                        dist = np.linalg.norm(np.array(list(observed_frontier)).reshape(-1, 2) - node.coords, axis=-1)
                        if dist.min() < UTILITY_RANGE + FRONTIER_CELL_SIZE:
                            to_update = True
                    if len(new_frontier) > 0:
                        dist = np.linalg.norm(np.array(list(new_frontier)).reshape(-1, 2) - node.coords, axis=-1)
                        if dist.min() < UTILITY_RANGE + FRONTIER_CELL_SIZE:
                            to_update = True
                    if to_update:
                        node.update_node_observable_frontiers(new_frontier, observed_frontier, updating_map_info)
    ...
    def a_star(self, start, destination, boundary=None, max_dist=None):
        ...
        while len(open_list) > 0:
            ...
            for neighbor_node_coords in node.neighbor_list:
                if self.nodes_dict.find(neighbor_node_coords) is None:
                    continue
...
class Node:
    ...
    def update_node_observable_frontiers(self, new_frontiers, observed_frontiers, updating_map_info):
