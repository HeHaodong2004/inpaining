...
from rl_model import PolicyNet, QNet
        ...
        while True:
            ...
            if curr_episode % 1 == 0 and len(experience_buffer[0]) >= MINIMUM_BUFFER_SIZE:
                ...
                for j in range(8):
                    ...
                    assert torch.all(current_index == critic_current_index)
                    assert torch.all(next_current_index == critic_next_current_index)
                    assert torch.all(current_edge == critic_current_edge)
                    assert torch.all(next_current_edge == critic_next_current_edge)
                    ...
                    logp = dp_policy(*critic_observation)
                    ...
                    with torch.no_grad():
                        next_logp = dp_policy(*critic_next_observation)
